...
export class FilmCardComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.reviews=this.http.get("http://localhost:4201/reviews").subscribe((x)=>{
      this.reviews=x
    })
  ...
  provaRecensione() {
    ...
    this.http.post<Review>('http://localhost:4201/reviews', this.review).subscribe(() => {
      this.http.get("http://localhost:4201/reviews").subscribe((x) => {
      this.reviews = x
    })
    })
